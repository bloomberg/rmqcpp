...
TEST(Methods_ConfirmSelect, QoSEncodeDecode)
{
    rmqamqpt::ConfirmSelect confirmSelect(true);
    ...
    rmqamqp::Framer::makeMethodFrame(
        &frame, channel, rmqamqpt::ConfirmMethod(confirmSelect));
    ...
    EXPECT_TRUE(m.is<rmqamqpt::ConfirmMethod>());
    const rmqamqpt::ConfirmMethod& bm = m.the<rmqamqpt::ConfirmMethod>();

    EXPECT_TRUE(bm.is<rmqamqpt::ConfirmSelect>());
    EXPECT_TRUE(bm.the<rmqamqpt::ConfirmSelect>().noWait());
}

...
TEST(ConfirmSelectOk, OpenEncodeDecode)
{
    rmqamqpt::ConfirmSelectOk selectOkMethod;

    rmqamqp::Framer framer;

    rmqamqpt::Frame frame;
    rmqamqp::Framer::makeMethodFrame(
        &frame, 5, rmqamqpt::ConfirmMethod(selectOkMethod));
    ...
    const rmqamqpt::ConfirmMethod& cm = m.the<rmqamqpt::ConfirmMethod>();

    EXPECT_TRUE(cm.is<rmqamqpt::ConfirmSelectOk>());
}

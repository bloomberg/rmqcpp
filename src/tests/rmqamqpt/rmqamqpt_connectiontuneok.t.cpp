...
TEST(ConnectionTuneOk, TuneOkEncodeDecode)
{
    rmqamqpt::ConnectionTuneOk tuneOkMethod(2048, 100, 5);

    rmqamqp::Framer framer;

    rmqamqpt::Frame frame;
    rmqamqp::Framer::makeMethodFrame(
        &frame, 0, rmqamqpt::ConnectionMethod(tuneOkMethod));
    ...
    EXPECT_TRUE(cm.is<rmqamqpt::ConnectionTuneOk>());

    EXPECT_THAT(cm.the<rmqamqpt::ConnectionTuneOk>().channelMax(), 2048);
    EXPECT_THAT(cm.the<rmqamqpt::ConnectionTuneOk>().frameMax(), 100);
    EXPECT_THAT(cm.the<rmqamqpt::ConnectionTuneOk>().heartbeatInterval(), 5);
}

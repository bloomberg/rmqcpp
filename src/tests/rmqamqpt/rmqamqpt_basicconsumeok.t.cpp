...
TEST(BasicConsumeOk, ConsumeOkEncodeDecode)
{
    rmqamqpt::BasicConsumeOk consumeOk("consumer-tag");

    uint16_t channel = 1337;
    rmqamqp::Framer framer;

    rmqamqpt::Frame frame;
    rmqamqp::Framer::makeMethodFrame(
        &frame, channel, rmqamqpt::BasicMethod(consumeOk));

    rmqamqp::Message received;
    EXPECT_THAT(framer.appendFrame(&channel, &received, frame),
                Eq(rmqamqp::Framer::OK));
    EXPECT_EQ(channel, 1337);
    ...
    EXPECT_TRUE(bm.is<rmqamqpt::BasicConsumeOk>());
    EXPECT_THAT(bm.the<rmqamqpt::BasicConsumeOk>().consumerTag(),
                Eq("consumer-tag"));
}

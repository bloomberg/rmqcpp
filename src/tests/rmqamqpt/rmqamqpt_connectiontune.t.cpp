...
TEST(ConnectionOpenTune, TuneEncodeDecode)
{
    rmqamqpt::ConnectionTune tuneMethod(2048, 100, 5);

    rmqamqp::Framer framer;

    rmqamqpt::Frame frame;
    rmqamqp::Framer::makeMethodFrame(
        &frame, 0, rmqamqpt::ConnectionMethod(tuneMethod));
    ...
    EXPECT_TRUE(cm.is<rmqamqpt::ConnectionTune>());

    EXPECT_THAT(cm.the<rmqamqpt::ConnectionTune>().channelMax(), 2048);
    EXPECT_THAT(cm.the<rmqamqpt::ConnectionTune>().frameMax(), 100);
    EXPECT_THAT(cm.the<rmqamqpt::ConnectionTune>().heartbeatInterval(), 5);
}

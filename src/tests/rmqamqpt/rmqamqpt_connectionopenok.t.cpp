...
TEST(ConnectionOpenOk, OpenEncodeDecode)
{
    rmqamqpt::ConnectionOpenOk openOkMethod;

    rmqamqp::Framer framer;

    rmqamqpt::Frame frame;
    rmqamqp::Framer::makeMethodFrame(
        &frame, 0, rmqamqpt::ConnectionMethod(openOkMethod));
    ...
    EXPECT_TRUE(cm.is<rmqamqpt::ConnectionOpenOk>());
}

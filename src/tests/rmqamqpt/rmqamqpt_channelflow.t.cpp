...
TEST(ChannelFlow, FlowEncodeDecode)
{
    rmqamqpt::ChannelFlow flowMethod(1);

    rmqamqp::Framer framer;

    rmqamqpt::Frame frame;
    rmqamqp::Framer::makeMethodFrame(
        &frame, 5, rmqamqpt::ChannelMethod(flowMethod));
    ...
    EXPECT_TRUE(cm.is<rmqamqpt::ChannelFlow>());
    EXPECT_EQ(cm.the<rmqamqpt::ChannelFlow>().activeFlow(), 1);
}

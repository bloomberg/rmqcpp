...
TEST(Methods_BasicCancelOk, CancelOkEncodeDecode)
{
    rmqamqpt::BasicCancelOk basicCancelOk("test-consumer-tag");
    ...
    rmqamqp::Framer::makeMethodFrame(
        &frame, channel, rmqamqpt::BasicMethod(basicCancelOk));
    ...
    EXPECT_TRUE(bm.is<rmqamqpt::BasicCancelOk>());
    EXPECT_EQ(bm.the<rmqamqpt::BasicCancelOk>().consumerTag(),
              "test-consumer-tag");
}

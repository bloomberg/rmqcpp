...
TEST(Methods_Queue_Unbind, QueueUnbindEncodeDecode)
{
    rmqt::FieldTable ft;
    ...
    QueueUnbind queueUnbindMethod(queue, exchange, routingKey, ft);

    rmqamqp::Framer framer;

    rmqamqpt::Frame frame;
    rmqamqp::Framer::makeMethodFrame(
        &frame, 0, rmqamqpt::QueueMethod(queueUnbindMethod));
    ...
    ASSERT_TRUE(qm.is<rmqamqpt::QueueUnbind>());
    EXPECT_EQ(queueUnbindMethod, qm.the<rmqamqpt::QueueUnbind>());
}

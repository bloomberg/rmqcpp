...
class MockDecoder : public Decoder {
  public:
    ...
    Decoder::ReturnCode
    getVector(bsl::vector<rmqamqpt::Frame>* outFrames, Unused, size_t bytes)
    {
...
class TestConnection : public Test {
  ...
  protected:
    void SetUp()
    {
        ...
        d_connection =
            bsl::shared_ptr<TestableAsioConnection>(new TestableAsioConnection(
                callbacks,
...
TEST_F(TestConnection, OneFrameRead)
{
    ...
    const size_t BYTES_READ  = 10;
    const size_t FRAME_COUNT = 1;
...
TEST_F(TestConnection, FiftyFramesRead)
{
    ...
    const size_t BYTES_READ  = 500;
    const size_t FRAME_COUNT = 50;
...
{
    const size_t NUMBER_OF_READS = 10;
    EXPECT_THAT(d_callbacks.consumeCount, Eq(0));

    const size_t BYTES_READ  = 2000;

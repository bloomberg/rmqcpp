...
class RmqTest:
    ...
    def start_perftest_executable(self, parameters) -> subprocess.Popen:
    ...
    def start_consumer(
        self, queue: str, additional_args: Optional[List[str]] = None
    ) -> subprocess.Popen:
        """
        Start a consumer executable with the provided queue and optional additional_args.
        Consumer is discovered via the `RMQ_CONSUMER_EXE` env var.

        Consumer must accept:
           --uri   | str: full amqp endpoint
           --queue | str: the name of the queue to consume from

        Any number of other arguments may be passed with additional_args as a list like: ["--arg1", "val1", "--arg2", "val2",...]

        """

        connection_string = f"amqp://{self.config.user}:{self.config.pwd}@{self.config.hostname}:{self.config.port}/{self.vhost}"

        cmd = [self.config.consumer_exe, "--queue", queue, "--uri", connection_string]
        if additional_args:

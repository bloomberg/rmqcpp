...
bsl::shared_ptr<rmqt::Endpoint> TestParameters::endpoint()
{
    if (!caCert.empty()) {
        bsl::shared_ptr<rmqt::SecurityParameters> securityParams;
        if ((!clientCert.empty()) && (!clientKey.empty())) {
            securityParams = bsl::make_shared<rmqt::MutualSecurityParameters>(
                caCert, clientCert, clientKey);
        }
        else {
            securityParams = bsl::make_shared<rmqt::SecurityParameters>(caCert);
        }
        return bsl::make_shared<rmqt::SecureEndpoint>(
            uri, vhost, static_cast<uint16_t>(tlsPort), securityParams);
    }
    else {
        return bsl::make_shared<rmqt::SimpleEndpoint>(
            uri, vhost, static_cast<uint16_t>(port));
    }
}

bsl::shared_ptr<rmqt::Credentials> TestParameters::credentials()
{
    return bsl::make_shared<rmqt::PlainCredentials>(user, password);
}
...
{
    bsl::shared_ptr<rmqt::Endpoint> endpoint = this->endpoint();

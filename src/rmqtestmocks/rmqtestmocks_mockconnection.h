...
#ifndef INCLUDED_RMQTESTMOCKS_MOCKCONNECTION
#define INCLUDED_RMQTESTMOCKS_MOCKCONNECTION
...
/// \brief Mocks rmqp::Connection and/or rmqa::Connection

class MockConnection : public rmqp::Connection {
  public:
    // CREATORS
    MockConnection();
    ~MockConnection() BSLS_KEYWORD_OVERRIDE;

    bsl::shared_ptr<rmqp::Connection> connection();

    rmqt::Future<rmqp::Connection> success();
    static rmqt::Future<rmqp::Connection> timeout();
    static rmqt::Future<rmqp::Connection> error();
  ...
  private:
    MockConnection(const MockConnection&) BSLS_KEYWORD_DELETED;
    MockConnection& operator=(const MockConnection&) BSLS_KEYWORD_DELETED;
...
#endif // ! INCLUDED_RMQTESTMOCKS_MOCKCONNECTION

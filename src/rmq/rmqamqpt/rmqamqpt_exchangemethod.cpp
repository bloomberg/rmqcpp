...
namespace rmqamqpt {
namespace {
BALL_LOG_SET_NAMESPACE_CATEGORY("RMQAMQPT.EXCHANGEMETHOD")

template <typename T>
bool decodeMethod(ExchangeMethod* exchangeMethod,
                  const uint8_t* data,
                  bsl::size_t dataLen)
{
    exchangeMethod->createInPlace<T>();
    return T::decode(&exchangeMethod->the<T>(), data, dataLen);
}

#define DECODE_METHOD(method)                                                  \
    case method::METHOD_ID:                                                    \
        return decodeMethod<method>(exchangeMethod, data, dataLen);

bool decodeExchangeMethodPayload(ExchangeMethod* exchangeMethod,
                                 uint16_t methodId,
                                 const uint8_t* data,
                                 bsl::size_t dataLen)
{
    switch (methodId) {
        DECODE_METHOD(ExchangeBind)
        DECODE_METHOD(ExchangeBindOk)
        DECODE_METHOD(ExchangeDeclare)
        DECODE_METHOD(ExchangeDeclareOk)
        default: {
            BALL_LOG_ERROR
                << "Failed to decode ExchangeMethod with unknown id: "
                << methodId;
...
struct ExchangeMethodEncoder {

    ExchangeMethodEncoder(Writer& os)
    : d_os(os)
    {
    }
...
rmqamqpt::Constants::AMQPMethodId ExchangeMethod::methodId() const
{
    return static_cast<rmqamqpt::Constants::AMQPMethodId>(
        this->apply(MethodIdFetcher()));
}

size_t ExchangeMethod::encodedSize() const
{
    return sizeof(uint16_t) + this->apply(MethodSizeFetcher());
}

bool ExchangeMethod::Util::decode(ExchangeMethod* exchangeMethod,
                                  const uint8_t* data,
                                  bsl::size_t dataLength)
{
    ...
    return decodeExchangeMethodPayload(exchangeMethod,
                                       methodId,
                                       data + sizeof(methodId),
                                       dataLength - sizeof(methodId));
}

void ExchangeMethod::Util::encode(Writer& output,
                                  const ExchangeMethod& exchangeMethod)
{
    exchangeMethod.apply(ExchangeMethodEncoder(output));
}

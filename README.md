...
## Rationale
`rmqcpp` provides a testable, data-safety-focused and async-capable API which strives to be easy to use.

Out of the box `rmqcpp` will never silently drop messages, and will always reconnect

### Features
This library has been built from experience learned from supporting other RabbitMQ libraries for many years. Using `rmqcpp` straight away ticks a lot of boxes in our internal RabbitMQ best-practices, as well as those suggested by [CloudAMQP](https://www.cloudamqp.com/blog/part1-rabbitmq-best-practice.html), and [AWS](https://docs.aws.amazon.com/amazon-mq/latest/developer-guide/best-practices-rabbitmq.html)

1. ❤ Heartbeats are always enabled, and are handled in the background.
    + Heartbeats catch many network faults, and some applications avoid them because existing libraries make heartbeats hard to implement. `rmqcpp` has these implemented out of the box.
...
4. ✉ Reliable Message Delivery on by default
    + Publisher confirmations. This ensures clients are aware when messages are owned by RabbitMQ, and avoids messages being silently blackholed.
    + Consumer acknowledgements. This avoids messages being silently dropped during restart/outages due to autoack.
    + Durable queues and persistent delivery mode ensure messages always persist during broker restarts and total datacentre shutdowns
...
6. ✂ Reconnect cancelled consumers
    + Very occasionally RabbitMQ consumers are cancelled by the broker. This case is rarely implemented properly when using other libraries. `rmqcpp` will redeclare any consumers which have been cancelled.
...
## Building
Build the project and run the tests (from source root directory)

Once you have set up the build dependencies on your machine, the project should
be buildable with the following commands:

1. `make setup` - This will initialise your environment, create the build

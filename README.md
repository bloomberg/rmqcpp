...
## Building
Build the project and run the tests (from your source root directory)

Once you have set up the build dependencies on your machine, the project should
be buildable using the following commands:

1. `make setup` - This will initialize your environment and create the build
   directory. It should only need to be run the first time you are setting up
   a build, except when bumping the dependency versions.
2. `make init` - This will do the CMake initialization, and should be run when
